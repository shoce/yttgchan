...
GOOS=linux GOARCH=amd64 go build -trimpath -o $HOME/gurusergeybugaev/
cp yttgchan.go $HOME/gurusergeybugaev/
cd $HOME/gurusergeybugaev/
git commit -am yttgchan
git reset $(git commit-tree HEAD^{tree} -m "yttgchan+ffmpeg")
git push -f

GOOS=linux GOARCH=amd64 go build -trimpath -o $HOME/denisdzen/
cp yttgchan.go $HOME/denisdzen/
*/
...
func HerokuUpdateYtLast(audioName string) error {
	req, err := http.NewRequest(
		"PATCH",
		HerokuVarsUrl,
		strings.NewReader(fmt.Sprintf(`{"YtLast": "%s"}`, audioName)),
	)
	if err != nil {
		return err
	}
...
func main() {
	...
	for vidnum, vid := range videos {
		...
		ytresp, err := YtCl.GetStreamContext(Ctx, vinfo, &audioFormat)
		if err != nil {
		...
		_, err = io.Copy(audioBuf, ytresp.Body)
		if err != nil {
			break
		}
		...
		err = HerokuUpdateYtLast(audioName)
		if err != nil {
			log("HerokuUpdateYtLast: %v", err)
			break
		}
